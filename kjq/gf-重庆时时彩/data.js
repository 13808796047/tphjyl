    ...
    client.query("insert into gygy_data(type, time, number, data) values(?,?,?,?)", [data.type, data.time, data.number, data.data], function(err, result) {
        if (err) {
            ...
            if (err.number == 1062) {
                ...
                log(conf['title'] + '第' + data.number + '期数据已经存在数据，延时10秒重新派奖');
                setTimeout(calcJ, 10000, data, true); //修改后
                try {
...
function setPj(sqls, data) {
	...
	console.log('开奖sqls开始：未去重');
    console.log(sqls);
    sqls = arr_unique(sqls);
    console.log('已去重：');
    console.log(sqls);
    console.log('开奖sqls结束；');
    var client = createMySQLClient();
    if (client == false) {
