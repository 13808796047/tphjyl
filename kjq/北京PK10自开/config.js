...
exports.openBeforTime=10; //提前2秒
// 出错时等待 15
exports.errorSleepTime=5; //等待5秒

// 时间监听执行间隔 
exports.runEveryTime=5; //等待5秒
...
exports.cp=[
	{   type:20,
		...
		option:{//时间执行配置
			...
			timeout:10000,
			method: 'GET',
			path: '/getinfo?type=20', //'/time.json'//  // Info0 定义在D:\webroot\lafei\Application\Home\Controller\WufencaiController.class.php 
			headers:{
...
/* 	var jd = JSON.parse(str);
	...
	var nowTime = parseInt((Date.parse( new Date())+'').substr(0,10));
		...
		client.query(sql, [type, match[1]], function(err, bets){
			if(err){
				...
				if(efFullData.number!='') {
					   ...
					   sbData(nowTime, efFullData['number'], efFullData['data'], efFullData['data'].replace(/,/g,''), efFullData['mytime']);
...
function gantData() {
	var base = ['01','02','03','04','05','06','07','08','09','10'];
	var data = '';
	var i = 0;
	var s = '';
	while(i < 10){
		s = base[GetRandomNum(0,9)];
		if(data.indexOf(s)  == -1){
			data = data === '' ? s : data+','+s;
			i++;
		}
