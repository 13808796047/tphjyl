...
function submitData(data, conf,callback){
	...
	// console.log(data);
	client.query('select data from gygy_data where type=? order by time desc limit 1',[data.type],function (err,result) {

		if (result[0].data != data.data){
	...
	client.query("insert into gygy_data(type, time, number, data) values(?,?,?,?)", [data.type, data.time, data.number, data], function(err, result){
	//client.query("call sp_getNewOpenData(?,?,?,?)", [conf.type, actionNo, 0,9], function(err, result){
        ...
        if(err){
            ...
            if(err.number==1062){
                ...
                restartTask(conf, conf.dosleep, true);
            }else{
