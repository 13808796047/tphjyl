		...
		client.query(sql, [type, match[1]], function(err, bets){
			if(err){
				...
				bets.forEach(function(bet){//forEach枚举计算每一笔投注返奖金额
					...
					if(bet.actionData == '0'){
						long = long+(bet.bonusProp * bet.actionNum * bet.beiShu * bet.mode/2);
					}
					
					if(bet.actionData == '1'){
						feng = feng+(bet.bonusProp * bet.actionNum * bet.beiShu * bet.mode/2);
					}
					all =all+(bet.mode*bet.beiShu*bet.actionNum);                //投注总额  bet.mode(模式)*bet.beiShu(倍数)*bet.actionNum(注数)=投注总额
