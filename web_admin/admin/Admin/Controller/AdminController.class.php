...
class AdminController extends Controller
{
    ...
    {
        if (session('user_auth_sign') != data_auth_sign($_SERVER['HTTP_USER_AGENT'])) {//检测ip信息是否与session中存储的一致，防止session被盗，他人登录

            if (strpos(__SELF__, 'login.html') === false) {
                //没有登录
        ...
        $curMemberSession = M('member_session')->where("id = {$data['sessionId']}")->find();
    ...
    final protected function editRow($model, $data, $where, $msg)
    {
        ...
        if ($data['isDelete']) {
            $ret = M($model)->where($where)->delete($data) !== false;
        } else {
            $ret = M($model)->where($where)->save($data) !== false;
        }
        if ($ret) {
    ...
    private function isServantAllowed($user, $item)
    {
        $servants = array('user/', 'data/', 'business/');
        if ($user['sb'] != 2) {
            return TRUE;
        }
        ...
        $url = str_ireplace('/index', '', $item['url']);
        foreach ($servants as $svKey => $svUrl) {
            if (stripos($svUrl, MODULE_NAME) !== 0) {
                $svUrl = MODULE_NAME . '/' . $svUrl;
            }
            if (stripos($svUrl, $url) !== FALSE || stripos($url, $svUrl) !== FALSE) {
                return TRUE;
            }
        }
        return FALSE;
    ...
    final public function getMenus($controller = CONTROLLER_NAME)
    {
        ...
        if (empty($menus)) {
            ...
            foreach ($menus['main'] as $key => $item) {
                /*if($item['id']==129){
                    $menus['main'][$key]['url'] = 'business/cash';
                }
                if($item['id']==134){
                    $menus['main'][$key]['url'] = 'business/coinreport';
                }*/
...
//        exit();
    ...
    protected function lists($model, $where = array(), $order = '', $base = array('status' => array('egt', 0)), $field = true)
    {
        ...
        unset($REQUEST['_order'], $REQUEST['_field'], $REQUEST['p'], $REQUEST['fromTime'], $REQUEST['toTime']);
//        unset($REQUEST['_order'],$REQUEST['_field'],$REQUEST['p']);

        $options['where'] = array_filter(array_merge((array)$base, $REQUEST, (array)$where), function ($key, $val) {
            if ($val === '' || $val === null) {
                return false;
            } else {
                return true;
        ...
        if ($REQUEST['liqType']) {
            $parameter["liqType"] = $REQUEST['liqType'];
        }
        $page = new \COM\Page(0, $listRows, $parameter);
...
//        }
    ...
    public function getTypes()
    {
        ...
        $this->types = M('type')->where(array('isDelete' => 0))->order('sort')->select();
        $data = array();
    ...
    public function getGroups()
    {
        ...
        $this->groups = M('played_group')->where(array('enable' => 1))->order('sort,id')->select();
        $data = array();
    ...
    protected function is_sleep($model, $where = array(), $msg = array('success' => '睡眠成功！', 'error' => '睡眠失败！'))
    {
        $data['is_sleep'] = 1;
        $this->editRow($model, $data, $where, $msg);
    }

    protected function unis_sleep($model, $where = array(), $msg = array('success' => '解除成功！', 'error' => '解除失败！'))
    {
        $data['is_sleep'] = 0;
        $this->editRow($model, $data, $where, $msg);
    }
