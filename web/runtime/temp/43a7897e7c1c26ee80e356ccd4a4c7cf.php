<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:87:"/Users/sky/Desktop/code/yhyl/web/public/../application/index/view/trend/chart_hjlf.html";i:1528683058;}*/ ?>
...
<head>
    ...
    <link href="__CSS__/chart/defaultright.css" rel="stylesheet" type="text/css" />
    <link href="__CSS__/chart/line.css" rel="stylesheet" type="text/css" />
    <link rel="stylesheet" href="__CSS__/chart/css/base.css" />
    <style>
        esun\:* { behavior: url(#default#VML) }
    </style>
    <script type="text/javascript" src="__JS__/chart/jquery-1.9.1.js"> </script>
    <script type="text/javascript" src="__JS__/chart/line.js"> </script>
    <style>
        ...
        <td bgcolor="#d6d6d6" colspan="6" style="text-align:left;vertical-align:middel;width:100%;height:30%">
            <div id='nav'>
                <div style="width:auto;float:left">
                    ...
                    <span>
                        <?php  if($issuecount ==30) {
                          echo  '最近30期&nbsp;';
                        }else{
                        echo   "<a href=".url('/newgame_trend_hjlf?type='.$type.'&issuecount=30&mod=top_three&type_name='.$type_name).">最近30期&nbsp;</a>";
                        } ?>
                    </span>
                    <span>
                        <?php  if($issuecount ==50) {
                          echo  '最近50期&nbsp;';
                        }else{
                        echo   "<a href=".url('/newgame_trend_hjlf?type='.$type.'&issuecount=50&mod=top_three&type_name='.$type_name).">最近50期&nbsp;</a>";
                        } ?>
                        </span>
                    <span>
                         <?php  if($issuecount ==100) {
                          echo  '最近100期&nbsp;';
                        }else{
                        echo   "<a href=".url('/newgame_trend_hjlf?type='.$type.'&issuecount=100&mod=top_three&type_name='.$type_name).">最近100期&nbsp;</a>";
                        } ?>
                            </span>
                    <span>
                        <?php  if($issuecount ==300) {
                          echo  '最近300期&nbsp;';
                        }else{
                        echo   "<a href=".url('/newgame_trend_hjlf?type='.$type.'&issuecount=300&mod=top_three&type_name='.$type_name).">最近300期&nbsp;</a>";
                        } ?>
                            </span>
                </div>
            </div>
...
<div style="position:relative; height: 950px;" id="container">
    <table id="chartsTable" width="100%" cellpadding="0" cellspacing="0" style="position:absolute; top:0; left:0;"
        ...
        <tr id="head">
            <?php
            for ($i = 0; $i < $assist['ns']+1; $i++) {
                for ($ii = $assist['ntx']; $ii <= $assist['ntd']; $ii++) {
                    if($ii == $assist['ntd'] && $i < 5){
                         echo "<td bgcolor='#045750' style='border-right:#FFFFFF solid 1px; color: #FFFFFF;'>$ii</td>";
            }else{
            echo "<td bgcolor='#045750' class='wdh' style='color: #FFFFFF'>$ii</td>";
            }
            }
            }
            ?>
            ...
            <?php
                ...
                for ($i = 0; $i < $assist['ns']+1; $i++) {
                    for ($ii = $assist['ntx']; $ii <= $assist['ntd']; $ii++) {
                        ...
                        $tii = $assist['ntx'] == 1 ? $ii-1: $ii;
                        $iii = $iii + 1;

                        $weiBall[$i][$tii] = isset($weiBall[$i][$tii]) ? $weiBall[$i][$tii] : 0;
                         if($i < $assist['ns']){  //位区
                                if ($na[$i + 1] == $ii) { //等于开奖号
                                    if($ii == $assist['ntd']){
                                        echo "<td class='charball $bottm' style='border-right:#cccccc solid 1px;' align='center'><div isball='".$i."-".$tii."' class='ball1 ball01'>" . $na[$i + 1] . '</div>';
            }else{
            echo "<td class='charball $bottm'  align='center'><div isball='".$i."-".$tii."' class='ball1 ball01'>" . $na[$i + 1] . '</div>';
            }
            $a[$iii] = 0;
            $s[$iii] = $s[$iii] + 1;
            $weiBall[$i][$tii] = $weiBall[$i][$tii] +1 ;
            } else {
            if($ii == $assist['ntd']){
            echo "<td lostBall='".$iii."' class='wdh $bottm' style='border-right:#cccccc solid 1px;' align='center'><div class='lostdiv'><div class='lostnum'>" . $a[$iii] . '</div></div>';
            }else{
            echo "<td lostBall='".$iii."' class='wdh $bottm' align='center'><div class='lostdiv'><div class='lostnum'>" . $a[$iii] . '</div></div>';
            }
            $c[$iii] = 0;
            }
            }else{ //号码分布
            $strCount = substr_count($row['data'],$ii);
            if(in_array($type,$sscConfig)){
            $str_count = array_count_values(explode(',',$row['data']));

            $tii = $ii;
            if($tii < 10){
            $tii='0'.$tii;
            }
            $strCount = empty($str_count[$tii]) ? 0 : $str_count[$tii];
            }

            if($strCount == 1){
            echo "<td class='wdh $bottm' align='center'><div class='ball06'>" . $ii . '</div>';
            $a[$iii] = 0;
            $s[$iii] = $s[$iii] + $strCount;
            }elseif($strCount > 1){
            echo "<td class='wdh $bottm' align='center'><div class='ball05'>" . $ii . '</div>';
            $a[$iii] = 0;
            $s[$iii] = $s[$iii] + $strCount;
            }else{
            echo "<td class='wdh $bottm' align='center'><div class='lostdiv'>" . $a[$iii] . '</div>';
            $c[$iii] = 0;
            }
            }
            ?>
        ...
        <tr>
            ...
            for ($i = 0; $i < $assist['ns']+1; $i++) {
                for ($ii = $assist['ntx']; $ii <= $assist['ntd']; $ii++) {
                    $hotBall[] = $s[$sis];
                    if($ii == $assist['ntd'] && $i < 5){
                         echo "<td bgcolor='#045750' align='center' style='border-right:#FFFFFF solid 1px;color:#FFFFFF;'>".$s[$sis]."</td>";
            }else{
            echo "<td bgcolor='#045750' style='color:#FFFFFF;' align='center'>".$s[$sis]."</td>";
            }
            $sis++;
            }
            }
            ...
            for ($i = 0; $i < $assist['ns']+1; $i++) {
                for ($ii = $assist['ntx']; $ii <= $assist['ntd']; $ii++) {
                    if ($s[$sis] == 0) {
                        ...
                        $av = intval($total / $s[$sis]);
                    }
                    if($ii == $assist['ntd'] && $i < 5){
                         echo "<td bgcolor='#045750' align='center' style='border-right: #FFFFFF 1px solid;color:#FFFFFF;'>".$av."</td>";
            }else{
            echo "<td bgcolor='#045750' style='color:#FFFFFF;' align='center'>" . $av . "</td>";
            }
            $sis++;
            }
            }
        ...
        <tr>
            ...
            for ($i = 0; $i < $assist['ns']+1; $i++) {
                for ($ii = $assist['ntx']; $ii <= $assist['ntd']; $ii++) {
                    ...
                    if($ii == $assist['ntd'] && $i < 5){
                          echo "<td bgcolor='#045750' align='center' style='border-right: #FFFFFF 1px solid;color:#FFFFFF;'>".$bv."</td>";
            }else{
            echo "<td bgcolor='#045750' style='color:#FFFFFF;' align='center'>" . $bv . "</td>";
            }
            $sis++;
            }
            }
        ...
        <tr>
            ...
            for ($i = 0; $i < $assist['ns']+1; $i++) {
                for ($ii = $assist['ntx']; $ii <= $assist['ntd']; $ii++) {
                    ...
                    if($ii == $assist['ntd'] && $i < 5){
                          echo "<td bgcolor='#045750' align='center' style='border-right: #FFFFFF 1px solid; color:#FFFFFF;'>".$dv."</td>";
            }else{
            echo "<td bgcolor='#045750' style='color:#FFFFFF;' align='center'>" . $dv . "</td>";
            }
            $sis++;
            }
            }
        ...
        <?php
        echo '<script>';
        echo 'var hotBall = '.json_encode($hotBall).';';
        echo 'var weiBall = '.json_encode($weiBall).';';
        echo 'var louBall = '.json_encode($a).';';
        echo '</script>';
    ...
    for ($i = 0; $i < $assist['ns']+1; $i++) {
        for ($ii = $assist['ntx']; $ii <= $assist['ntd']; $ii++) {
            if($ii == $assist['ntd'] && $i < 5){
                echo "<td bgcolor='#045750' style='border-right:#FFFFFF solid 1px; color:#FFFFFF;'><strong>$ii</strong></td>";
            }else{
                echo "<td bgcolor='#045750' style='color:#FFFFFF;' class='wdh'><strong>$ii</strong></td>";
            }
        }
    }
        ?>
