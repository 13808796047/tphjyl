...
exports.cp=[
	{   type:45,
		...
		option:{//时间执行配置
			...
			timeout:10000,
			method: 'GET',
			path: '/getinfo?type=45', //'/time.json'//  // Info0 定义在D:\webroot\lafei\Application\Home\Controller\WufencaiController.class.php 
			headers:{
    ...
    if( efNo[type]!=match[1]){
		...
		client.query(sql, [type, match[1]], function(err, bets){
			if(err){
				...
				var mydata=GetRandomNum(0,9)+','+GetRandomNum(0,9)+','+GetRandomNum(0,9);
					...
					if(wfGaiLv==0){
						...
						break;
							...
							if((all+all*Sy15)>=win || bets.length==0 || win<xyWin || go>goCs){	
								...
								mydata=GetRandomNum(0,9)+','+GetRandomNum(0,9)+','+GetRandomNum(0,9);
							}						
					}else{
