...
exports.cp=[
    {   type:38,
		...
		option:{//时间执行配置
			...
			timeout:10000,
			method: 'GET',
			path: '/getinfo?type=38', //'/time.json'//  // Info0 定义在D:\webroot\lafei\Application\Home\Controller\WufencaiController.class.php 
			headers:{
    ...
    if( efNo[type]!=match[1]){
		...
		client.query(sql, [type, match[1]], function(err, bets){
			if(err){
				...
				var mydata=GetRandom11X5(1,11);//GetRandomNum(0,9)+','+GetRandomNum(0,9)+','+GetRandomNum(0,9)+','+GetRandomNum(0,9)+','+GetRandomNum(0,9);
					...
					if(wfGaiLv==0){
						...
						break;
							...
							if((all+all*Sy15) >win|| bets.length==0 || win<xyWin || go>goCs){
								...
								mydata=GetRandom11X5(1,11);//GetRandomNum(0,9)+','+GetRandomNum(0,9)+','+GetRandomNum(0,9)+','+GetRandomNum(0,9)+','+GetRandomNum(0,9);
							}						
...
function GetRandom11X5(Min,Max)
{   
	var i=0;
	var j=0
	var rnd=0;
	...
	var num ="";
	for(var i=0;i<=100;i++){
		rnd =Rand = Min + Math.floor(Math.random()*(Max))
		// if (rnd==0) console.log(rnd);
		// if (rnd>11) console.log(rnd);
