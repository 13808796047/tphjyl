$(function () {
    ...
    $('a.code').live('click', function () {
        ...
        var $this = $(this);
        if ($game.attr('action') == 'sscBD') {
            if ($this.is('.checked')) {
                $this.removeClass('checked');
                $('a.code').removeClass('checked');
            } else {
                $('a.code').removeClass('checked');
...
function gameKanJiangDataC(diffTime, actionNo) {
    ...
    var $dom = $('#time');
    if (diffTime < 7) {
        $("#bet-confirm-mobile").text('本期停止投注中...');
        $('#bet-confirm-mobile').unbind('click');
    }
    if (diffTime <= 0) {
...
function loadKjData() {
    ...
    var now = Math.random();
    $.ajax({
        url: "/index.php?s=/mobile/game/getLastKjData", //请求的url地址
        dataType: "json", //返回格式为json
        data: {"type": type, time: now}, //参数值
        type: "GET", //请求方式 get 或者post
        /*$.getJSON('/index.php?s=/home/index/getLastKjData/type/' + type, {
         dataType: 'json',
