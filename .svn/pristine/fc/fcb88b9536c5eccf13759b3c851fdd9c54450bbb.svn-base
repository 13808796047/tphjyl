...
function calcJbak(data, flag){
	var client=createMySQLClient();
	var pl = 0;
	sql="select value from gygy_params  where name= ? limit 1";
	client.query(sql,['gc_pt_kuisuan'],function(err,p){
		if(err){
			pl =0
		}else{
			try{
			...
			clacJNow(data, flag,pl);
		}
	});
	client.end();
}

function clacJNow(data, flag,pl){
	var client=createMySQLClient();
	...
	client.query(sql, [data.type, data.number], function(err, bets){
		if(err){
			...
			bets.forEach(function(bet){
				...
				log('亏损控制赔率：'+pl);
				if(pl > 0){
					var ykk = zjCount-bet.bets_money; //官彩官方不亏才派奖
					log('亏损实际赔率：'+ykk);
					if(pl>=ykk){
