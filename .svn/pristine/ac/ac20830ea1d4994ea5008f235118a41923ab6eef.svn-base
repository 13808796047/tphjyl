    ...
    public function group() {
		...
		if(IS_POST)
		{
			...
			foreach($post as $key=>$value)
			{
				...
				$d['value'] = $value;
				$str2 = $key.':'.$value.',';
				if(($return = $params->where($map)->save($d)) === false)
					$this->error('网站设置失败');
			}
	...
	public final function switchPlayedStatus(){
		...
		{			
            ...
            if($data){
				...
				if($data['is_official_open']=='on')
					$data['is_official_open'] =1;
				else
					$data['is_official_open'] =0;
