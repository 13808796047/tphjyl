...
class ActionController extends AdminController {
    ...
    public function adminLgnLog(){
		...
		$username = I('username');
		if(isset($username)){
            $where = 's.uid=m.uid and m.admin=1 and m.username like "%'.$username.'%"';
    	}
		else{
		    $where = 's.uid=m.uid and m.admin=1';
		}
        ...
        $field = 's.*,m.username as username';
        $list = $Model->table('__MEMBER_SESSION__ s,__MEMBERS__ m')->where($where)->field($field)->order('s.id desc')->page($pageIndex,$listRows)->select();
        $total = $Model->table('__MEMBER_SESSION__ s,__MEMBERS__ m')->where($where)->count();
...
//        $this->recordList($list);
    ...
    public function adminlog(){
        ...
        $list   = M('admin_log')->where($map)->order('id desc')->page($pageIndex,$listRows)->select();
        $total = M('admin_log')->where($map)->count();
        $total = $total > 0 ? $total : 0;
		//dump(M('member_session')->getLastSql());
        //$this->recordList($list);
        $request    =   (array)I('request.');
