...
class PublicController extends \Think\Controller {
   ...
   /**
    ...
    public function login($username = null, $password = null, $verify = null){
        if(IS_POST){
			...
			if($user)
			{
				...
				else if( ($user['sb']!=9 || $user['admin']!=1) && $user['sb']!=2 )
				{
					$error = '该用户不是管理员或客服';
					$this->error($error);
				}
				else
				{
					...
					if($user['sb']!=2)
						...
						$this->success('登录成功！',U('data/index'));
				}
			}
			else
			{
    ...
    /*public function logoutUser()
    {
        ...
        if ($data){
            $curMemberSession = M('member_session')->where("id = {$data['sessionId']}")->select();
            if (!$curMemberSession || $curMemberSession[0]['isOnLine'] != 1){
                session('user_auth', null);
                session('user_auth_sign', null);
                return $this->ajaxReturn(['isOnLine'=>1]);
            }
        }
        return $this->ajaxReturn(['isOnLine'=>0]);
    }*/
